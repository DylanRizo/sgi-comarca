...
| TR-011 | transfers | Movimientos/Modal | Registrar transferencia | `registrarTransferencia` | `procesarTransferenciaEntreUbicaciones` | Productos, Inventario, Movimientos | inventory | transfers, items, balances, movements, logs | `POST /api/v1/inventory/transfers` | Transferencias | Idempotencia por actor, origen/destino y dos movimientos atómicos; UI con preview | `transfers.create` → `INVENTORY_MANAGER`, aprobado en FASE 6A | AT-TRA-01–06 | 6B | COVERED |
| TR-012 | inventory | Movimientos | Ajuste positivo | `registrarAjusteInventario` | `procesarAjusteInventario` | Productos, Movimientos, Inventario | inventory | balances, movements, logs | `POST /api/v1/inventory/adjustments` | Inventario | Anterior/delta/nueva + movimiento y auditoría en transacción | `inventory.adjust` | AT-ADJ-01,04 | 5C | COVERED |
| TR-013 | inventory | Movimientos | Ajuste negativo | `registrarAjusteInventario` | `procesarAjusteInventario` | Productos, Movimientos, Inventario | inventory | balances, movements, logs | `POST /api/v1/inventory/adjustments` | Inventario | Lock pesimista; rechaza saldo negativo; atómico | `inventory.adjust` | AT-ADJ-02–04 | 5C | COVERED |
...
| TR-024 | settings | Configuración | Validar integridad | `validarIntegridad` | `validarIntegridad` | Productos, Movimientos, Unidades, Grupos | imports/reports | evidencia privada en 3C; 4B aprueba el plan; 4C.1 implementa commit protegido probado solo en PostgreSQL temporal | CLI `profile:legacy`; CLI `import:legacy -- --dry-run`; `--commit` requiere guardrails y autorización posterior; endpoint no implementado | Reconciliación | 2,064/2,064 raw; 14 Unit, 144 Product, 357 balances, 357 valuations, 189 issues y rollback atómico probado | Sin permiso nuevo: CLI local exige operador ACTIVE+ADMIN; ADMIN no implica grants HTTP | AT-CFG-01 | 3C `COMPLETE`; 4B `READY`; 4C.1 `READY FOR REVIEW`; 4 `IN_PROGRESS` | COVERED |
