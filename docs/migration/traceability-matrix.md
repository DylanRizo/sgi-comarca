...
| TR-024 | settings | Configuración | Validar integridad | `validarIntegridad` | `validarIntegridad` | Productos, Movimientos, Unidades, Grupos | imports/reports | evidencia privada en 3C; 4B simula Unit/Product/Balance/Valuation solo en PostgreSQL temporal | CLI `profile:legacy`; CLI `import:legacy -- --dry-run`; endpoint futuro no implementado | Reconciliación | 2,064/2,064 raw; 14 Unit, 144 Product, 357 balances y 357 valuations; commit rechazado | Permiso explícito pendiente para cualquier operación futura; ADMIN no implica grant | AT-CFG-01 | 3C `COMPLETE`; 4B `READY`; 4 `IN_PROGRESS` | COVERED |
