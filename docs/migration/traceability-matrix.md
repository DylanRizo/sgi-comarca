...
| TR-024 | settings | Configuración | Validar integridad | `validarIntegridad` | `validarIntegridad` | Productos, Movimientos, Unidades, Grupos | imports/reports | evidencia privada en 3C; entidades técnicas simuladas en PostgreSQL temporal desde 4A | CLI `profile:legacy`; CLI `import:legacy -- --dry-run`; endpoint futuro no implementado | Reconciliación | Perfil de 9 hojas; 4A preserva 2,064/2,064 raw y rechaza commit | Permiso explícito pendiente para cualquier operación futura; ADMIN no implica grant | AT-CFG-01 | 3C `COMPLETE`; 4 `IN_PROGRESS` | COVERED |
