...
| 6 | Catálogos e inventario — `IN_PROGRESS` (`6B IMPLEMENTATION`) | Grant y persistencia aprobados; API/historial/transferencia implementados y UI en validación; primera transferencia staging queda en gate separado; movimientos legacy no importados | concurrencia, stock no negativo, flujos atómicos y gate separado de escritura staging |
