...
| 6 | Catálogos e inventario — `IN_PROGRESS` (`6B COMPLETE`) | Grant, persistencia, API, historial y UI de transferencias completos; primera transferencia staging queda en gate separado; movimientos legacy no importados | stock receipts y cualquier alcance restante de inventario continúan separados; escritura staging requiere gate explícito |
